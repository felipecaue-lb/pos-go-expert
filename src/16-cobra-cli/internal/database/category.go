...
func (c *Category) FindByCourseID(courseID string) (Category, error) {
	...
	err := c.db.QueryRow("SELECT c.id, c.name, c.description FROM categories c JOIN courses co ON c.id = co.categoryID WHERE co.id = ?", courseID).Scan(&id, &name, &description)
	if err != nil {
		return Category{}, err
	}
